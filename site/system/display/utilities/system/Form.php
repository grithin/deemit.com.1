...
class Form {
	...
	///callbacks for final manipulation of field value
	static $valueCallbacks = array();
	...
	*/
	static function to_input($name, $value, $useArray=false){
	...
	static function to_params($name,$value){
		return self::to_input($name,$value,true);
	}
	
	/// resolves the value for a given form field name, returning a reference to the value
	static function &resolveValue($name, &$value, $behavior=null){
		...
		if(self::$valueCallbacks){
			foreach(self::$valueCallbacks as $callback){
	...
	static function select($name, $options, $value = null, $x = null){
		...
		if($detailedOptions){
			foreach ($detailedOptions as $k=>$details){
				...
				$field .= '<option '.($details['selected']?'selected=1':null).' value="'.htmlspecialchars($k).'" '.self::additionalAttributes($details['x']).'>'.htmlspecialchars($details['display']).'</option>';
