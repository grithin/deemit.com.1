...
//+	load controllers and page utilities{
		if(!self::$stopControls){
			...
			if(Config::$x['loadPageUtilities']){
				//try loading the page utilities
				self::getFirstUtility(Config::$x['instanceFolder'].'utilities/section/');
				self::getFirstUtility(Config::$x['instanceFolder'].'display/utilities/section/');
			}
...
//+	}
	...
	///get the first token based utility (if config page utilities is on)
	private static function getFirstUtility($base){
		$tokens = self::$urlTokens;
		while($tokens){
			if(files::incOnce($base.implode('/',$tokens).'.php')){
				return;
			}
			array_pop($tokens);
		}
