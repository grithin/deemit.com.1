...
class Page{
	...
	static function success($message,$name=null,$context=self::defaultContext){
	...
	*/
	...
	@param	rules	string or array	As a string, is interpreted as either a PageTool method, a InputValidator method, or a InputFilter method.  String form has the following forms:
		...
		name indicates standard input type, usually is expanded to multiple validators and filters
		
		type:method strings can be prefixed with "!" to indicate there should be a break (no more rules for that field applied) on error for current and following rules
		type:method strings can be prefixed with "!!" to indicate there should be a break (no more rules at all applied) on error for current and following rules
		...
		Examples:
			'v:email|bob.com,customClass:method|param1;param2',
			array('f.name',array('v.available',array(...),param2,param3))
	*/
	static function applyFilterValidateRules($field, &$value, $rules, $context){
		...
		for($i=0;$i<count($rules);$i++){
			...
			try{
				...
				//not actually an error, but a stopping validator
				if($e->getCode() == InputException::breakPass){
					break;
				}
				...
				if($e->getCode() == InputException::breakError || $break){
					break;
				}
			}
		}
...
if(!Config::$x['inScript'] || Config::$x['scripgGetsStdin']){
	...
	#in multipart forms php://stdin not populated, must use $_POST
	if(!$input['post'] && $_POST){
